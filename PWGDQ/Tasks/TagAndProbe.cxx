...
void initLog(const std::string& filename) {
    ...
    // file viene cancellato alla chiusura di clearFile
}
...
struct AnalysisTagAndProbe {
  ...
  int count_All = 1;
  int count_PassingProbes = 1;
  int count_FailingProbes = 1;
  ...
  void runTagAndProbe(TEvents const& events, Preslice<TTrackAssocs>& preslice, TTrackAssocs const& assocs, TTracks const& /*tracks*/)
  {
    ...
    /*const char* filename = "trackType.txt";
    ...
    char ftxt[200];
    sprintf(ftxt, "trackType_All.txt");
    std::ofstream ftxt_trackType_All(ftxt, std::ios::app);
    sprintf(ftxt, "trackType_PassingProbes.txt");
    std::ofstream ftxt_trackType_PassingProbes(ftxt, std::ios::app);
    sprintf(ftxt, "trackType_FailingProbes.txt");
    std::ofstream ftxt_trackType_FailingProbes(ftxt, std::ios::app);
      ...
      for (auto& event : events) {
        ...
        for (auto& [a1, a2] : o2::soa::combinations(groupedAssocs, groupedAssocs)) {
          if constexpr (TPairType == VarManager::kDecayToMuMu) {
            ...
            sign1 = t1.sign();
            sign2 = t2.sign();
            //LOGP(info, "[{}, {}, {}] -> ({} , {})", event.posX(), event.posY(), event.posZ(), t1.sign(), t2.sign());
            VarManager::FillPair<TPairType, TTrackFillMap>(t1, t2);

            for (int icut = 0; icut < ncuts; icut++) {
              if (sign1 * sign2 < 0) {
                fHistMan->FillHistClass(histNames[icut][0].Data(), VarManager::fgValues);
                LOGP(info, "{}: Pos Event [{}, {}, {}] -> muons sign ({} , {}) - trackType muons ({},{}) - muons pt - ({},{}) - muons eta ({},{}) - muons phi ({},{})", count_All, event.posX(), event.posY(), event.posZ(), t1.sign(), t2.sign(), t1.trackType(), t2.trackType(), t1.pt(), t2.pt(), t1.eta(), t2.eta(), t1.phi(), t2.phi());
                ftxt_trackType_All << count_All <<": Pos Event [" << event.posX() << ", " << event.posY() << ", " << event.posZ() << "]" 
                << " -> muons sign (" << t1.sign() << " , " << t2.sign() << ")\n" 
                << " - trackType muons (" << static_cast<int>(t1.trackType()) << "," << static_cast<int>(t2.trackType())<< ")\n" 
                << " - muons pt (" << t1.pt() << " , " << t2.pt() << ")\n" 
                << " - muons eta (" << t1.eta() << " , " << t2.eta() << ")\n" 
                << " - muons phi (" << t1.phi() << " , " << t2.phi() << ")\n\n" << std::endl;  
                ...
                if (static_cast<int>(t1.trackType()) == 3) { // t1 is the tag (track MCHMID)
                  if (static_cast<int>(t2.trackType()) == 3) { // t2 is the passing probe (track MCHMID)
                    ...
                    ftxt_trackType_PassingProbes << count_PassingProbes << ": Pos Event [" << event.posX() << ", " << event.posY() << ", " << event.posZ() << "]" 
                    << " -> muons sign (" << t1.sign() << " , " << t2.sign() << ")\n" 
                    << " - trackType muons (" << static_cast<int>(t1.trackType()) << "," << static_cast<int>(t2.trackType())<< ")\n" 
                    << " - muons pt (" << t1.pt() << " , " << t2.pt() << ")\n" 
                    << " - muons eta (" << t1.eta() << " , " << t2.eta() << ")\n" 
                    << " - muons phi (" << t1.phi() << " , " << t2.phi() << ")\n\n" << std::endl; 
                    count_PassingProbes = count_PassingProbes +1;
                  } else if (static_cast<int>(t2.trackType()) == 4){ // t2 is the failing probe (MCHStandalone)
                    fHistMan->FillHistClass(histNames[icut][2].Data(), VarManager::fgValues);
                    ftxt_trackType_FailingProbes << count_FailingProbes << ": Pos Event [" << event.posX() << ", " << event.posY() << ", " << event.posZ() << "]" 
                    << " -> muons sign (" << t1.sign() << " , " << t2.sign() << ")\n" 
                    << " - trackType muons (" << static_cast<int>(t1.trackType()) << "," << static_cast<int>(t2.trackType())<< ")\n" 
                    << " - muons pt (" << t1.pt() << " , " << t2.pt() << ")\n" 
                    << " - muons eta (" << t1.eta() << " , " << t2.eta() << ")\n" 
                    << " - muons phi (" << t1.phi() << " , " << t2.phi() << ")\n\n" << std::endl; 
                    count_FailingProbes = count_FailingProbes +1;
                  } else {
                    continue;
                  }
                }
    ...
    ftxt_trackType_All.close();
    ftxt_trackType_PassingProbes.close();
    ftxt_trackType_FailingProbes.close();
